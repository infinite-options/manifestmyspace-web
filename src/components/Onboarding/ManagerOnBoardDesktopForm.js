...
import { Box, Button, TextField, Typography, Stack, Checkbox, Grid, CircularProgress, Backdrop, Paper, Select, MenuItem } from "@mui/material";
...
const ManagerOnBoardDesktopForm = ({ profileData, setIsSave }) => {
  ...
  const handleFeeChange = (event, id) => {
    ...
    setFees((prevFees) => prevFees.map((fee) => (fee.id === id ? { ...fee, [name]: value } : fee)));
  };
