...
function POProfileName() {
  ...
  const {
    owner_first_name,
    update_owner_first_name,
    owner_last_name,
    update_owner_last_name,
    owner_photo,
    update_owner_photo,
  } = useMyContext();
  ...
  useEffect(() => {
    update_owner_first_name(user.first_name);
    update_owner_last_name(user.last_name);
  }, []);
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        >
          ...
          <Box sx={{ paddingTop: "8%" }} />
          <Stack direction="row" justifyContent="center">
            {owner_photo && owner_photo.image ? (
              <img
                key={Date.now()}
                src={owner_photo.image}
                style={{
                  width: "121px",
                  height: "121px",
                  objectFit: "cover",
