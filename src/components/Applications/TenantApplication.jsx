...
export default function TenantApplication() {
  ...
  useEffect(() => {
    const getTenantProfileInformation = async () => {
      const response = await fetch(
        `https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/tenantProfile/${getProfileId()}`
      );
  ...
  function handleApplicationSubmit() {
    ...
    const annoucementsResponse = fetch(
      `https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/announcements/${getProfileId()}`,
      {
        ...
        body: JSON.stringify({
          ...
          announcement_msg:
            "You have a new tenant application for your property",
    ...
    leaseApplicationData.append(
      "lease_children",
      tenantProfile?.tenant_children_occupants
    );
    leaseApplicationData.append(
      "lease_pets",
      tenantProfile?.tenant_pet_occupants
    );
    leaseApplicationData.append(
      "lease_vehicles",
      tenantProfile?.tenant_vehicle_info
    );
    ...
    if (getProfileId() !== null && getProfileId() !== undefined) {
      ...
      // Handle the case when getProfileId() is null or undefined
      console.error("getProfileId() is null or undefined");
    }

    const leaseApplicationResponse = fetch(
      `https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/leaseApplication`,
      {
        method: "POST",
              ...
              {petOccupants &&
                petOccupants.map((occupant, index) => (
                  <Typography
                    ...
                    key={index}
                  >
                    {occupant.name} {occupant.type} | {occupant.relationship} |
                    DOB: {occupant.dob}
                  </Typography>
                ))}
            </Grid>
