...
import { Card, CardContent, MenuItem, Select, FormControl, InputLabel, OutlinedInput, Typography, Button, Box, Stack, Grid } from "@mui/material";
...
export default function MaintenanceRequestNavigatorNew({
  ...
  const [priorities, setPriorities] = useState(requestData.map((request) => request.maintenance_priority || "Medium"));
                ...
                >
                  <ImageList sx={{ display: "flex", flexWrap: "nowrap" }} cols={5}>
                    {images.map((image, index) => (
                      ...
                      >
                        <img src={image} alt={`maintenance-${index}`} style={{ height: "150px", width: "150px", objectFit: "cover" }} />
                      </ImageListItem>
                    ))}
                  </ImageList>
                </Box>
                  ...
                  <Grid container spacing={2} sx={{ marginTop: 1 }}>
                    <Grid item xs={6}>
                      ...
                      <Typography variant='body1' sx={{ display: "flex", alignItems: "center", marginBottom: 1 }}>
                        <strong style={{ minWidth: "150px" }}>Owner:</strong> {data?.owner_first_name + " " + data?.owner_last_name}
                      </Typography>
                    </Grid>
                    <Grid item xs={6}>
                      <Typography variant='body1' sx={{ display: "flex", alignItems: "center", marginBottom: 5 }}>
                        <strong style={{ minWidth: "150px" }}>Reported On:</strong> {dayjs(data?.maintenance_request_created_date).format("MM-DD-YYYY")}
                      </Typography>
                      <Typography variant='body1' sx={{ display: "flex", alignItems: "center", marginBottom: 5 }}>
                        <strong style={{ minWidth: "150px" }}>Days Open:</strong> {dayjs().diff(dayjs(data?.maintenance_request_created_date), "day")} days
                      </Typography>
                      <Typography variant='body1' sx={{ display: "flex", alignItems: "center", marginBottom: 1 }}>
                ...
                <Grid container sx={{ padding: "0px" }}>
                  ...
                  <Grid container sx={{ padding: "0px" }}>
                    {data.maintenance_request_status !== "NEW" && maintenanceQuotes.length > 0 ? (
                      <QuoteDetails maintenanceItem={data} navigateParams={navigateParams} maintenanceQuotesForItem={maintenanceQuotes} />
                    ) : null}
                  </Grid>
                </Grid>
              </Box>
