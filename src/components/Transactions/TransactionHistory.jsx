...
import { descendingComparator as descendingComparator, getComparator as getComparator, stableSort as stableSort } from "../utils/helper";
...
export default function TransactionHistory(props) {
  console.log("In TransactionHistory.jsx");
  ...
  function EnhancedTableHeadOutgoingPayments(props) {
    ...
    return (
      <TableHead>
        <TableRow sx={{ borderBottom: "1px solid #000000" }}>
          {paymentsOutgoingHeadCell.map((headCell) => (
            <TableCell key={headCell.id} sx={{ fontSize: 18 }} align="left" size="small" sortDirection={orderBy === headCell.id ? order : false}>
              <TableSortLabel align="center" active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : "asc"} onClick={createSortHandler(headCell.id)}>
                {headCell.label}
                {orderBy === headCell.id ? (
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          >
            ...
            />
            {loading && <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>{loading && <CircularProgress color="inherit" />}</div>}
            {!loading && (
              <Stack direction="row" justifyContent="center">
                <Table>
                  <EnhancedTableHeadOutgoingPayments order={order} orderBy={orderBy} onRequestSort={handleRequestSort} rowCount={history ? history.length : 0} />
                          ...
                          return row.purchase_status === "COMPLETED" || row.purchase_status === "PAID" ? (
                            ...
                            >
                              <TableCell align="left" className={classes.cell_short} sx={{ fontSize: 18 }}>
                                {row.purchase_date !== null ? moment(row.purchase_date.substring(0, 10)).format("MM/DD/YY") : "Not Available"}
                              </TableCell>
                              <TableCell align="left" className={classes.cell_short} sx={{ fontSize: 18 }}>
