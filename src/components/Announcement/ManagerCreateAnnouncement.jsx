...
import { Grid, Box, ThemeProvider, Radio, RadioGroup, FormControlLabel, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Checkbox } from "@mui/material";
...
export default function ManagerCreateAnnouncement() {
  ...
  useEffect(() => {
    ...
    axios.get(`https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/properties/${getProfileId()}`).then((res) => {
      ...
      const owners = properties.filter(
        (property, index, self) => index === self.findIndex((a) => a.owner_first_name === property.owner_first_name && a.owner_last_name === property.owner_last_name)
      );

      const ownersWithProperties = owners.map((owner) => {
        const ownerProperties = properties.filter((property) => property.owner_first_name === owner.owner_first_name && property.owner_last_name === owner.owner_last_name);
      ...
      const tenants = properties
        ...
        .filter((property, index, self) => index === self.findIndex((a) => a.tenant_first_name === property.tenant_first_name && a.tenant_last_name === property.tenant_last_name));
      const tenantsWithProperties = tenants.map((tenant) => {
        const tenantProperties = properties.filter((property) => property.tenant_first_name === tenant.tenant_first_name && property.tenant_last_name === tenant.tenant_last_name);
        ...
        <form onSubmit={handleSendAnnouncement}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              ...
              <TextField fullWidth label='' value={announcementTitle} onChange={(e) => setAnnouncementTitle(e.target.value)} />
            </Grid>
            <Grid item xs={12}>
              <Typography
                sx={{
              ...
              <TextField fullWidth multiline rows={4} label='' value={announcementMessage} onChange={(e) => setAnnouncementMessage(e.target.value)} />
            </Grid>
            <Grid item xs={12}>
              <RadioGroup aria-label='announcementType' name='announcementType' value={selectedOption} onChange={handleOptionChange} row>
                <FormControlLabel value='tenants_by_name' control={<Radio />} label='Tenants By Name' />
                <FormControlLabel value='owners_by_name' control={<Radio />} label='Owners By Name' />
                <FormControlLabel value='applicants_by_name' control={<Radio />} label='Applicants By Name' />
                <FormControlLabel value='tenants_by_property' control={<Radio />} label='Tenants By Property' />
              </RadioGroup>
            </Grid>
            <Grid item xs={12}>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {tenantsData.map((tenant, index) => (
                            <TenantRow key={index} tenant={tenant} isSelected={selectedUsers.includes(tenant)} handleCheckboxChange={handleCheckboxChange} />
                          ))}
                        </TableBody>
                      </Table>
                    </TableContainer>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {ownersData.map((owner, index) => (
                            <OwnerRow key={index} owner={owner} isSelected={selectedUsers.includes(owner)} handleCheckboxChange={handleCheckboxChange} />
                          ))}
                        </TableBody>
                      </Table>
                    </TableContainer>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {tenantsByPropertyData.map((property, index) => (
                            <TenantByPropertyRow key={index} property={property} isSelected={selectedUsers.includes(property)} handleCheckboxChange={handleCheckboxChange} />
                          ))}
                        </TableBody>
                      </Table>
                    </TableContainer>
  ...
  return (
    <TableRow>
      <TableCell padding='checkbox'>
        <Checkbox checked={isSelected} onChange={handleChange} color='primary' inputProps={{ "aria-label": "select user" }} />
      </TableCell>
      <TableCell>{`${tenant.tenant_first_name} ${tenant.tenant_last_name}`}</TableCell>
      <TableCell>
        {tenant.properties_list.map((property, index) => (
          <Box key={index}>{`${property.property_address}, Unit - ${property.property_unit}, ${property.property_city}, ${property.property_state}`}</Box>
        ))}
      </TableCell>
    </TableRow>
  );
