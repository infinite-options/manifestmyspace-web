...
const PropertyCard = (props) => {
	...
	const fetchData = async () => {
		...
		if (data !== 'No records for this Uid') {
			...
			setContractStartDate(
				contractData['contract_start_date'] ? dayjs(contractData['contract_start_date']) : dayjs()
			);
			setContractEndDate(
				contractData['contract_end_date']
					? dayjs(contractData['contract_end_date'])
					: contractStartDate.add(1, 'year').subtract(1, 'day')
			);
			setContractStatus(contractData['contract_status'] ? contractData['contract_status'] : '');
			setContractAssignedContacts(
				contractData['contract_assigned_contacts'] ? JSON.parse(contractData['contract_assigned_contacts']) : []
			);
	...
	const setContractDetails = () => {
		if (allContracts !== null && allContracts !== undefined) {
			...
			if (contractData) {
				...
				setContractFees(contractData['contract_fees'] ? JSON.parse(contractData['contract_fees']) : []);
				const oldDocs = contractData['contract_documents']
					? JSON.parse(contractData['contract_documents'])
					: [];
				setPreviouslyUploadedDocs(oldDocs);
				const contractDoc = oldDocs?.find((doc) => doc.type === 'contract');
				if (contractDoc) {
	...
	const handleSendQuoteClick = () => {
		...
		if (
			!isValidDate(contractEndDate.format('MM-DD-YYYY')) ||
			!isValidDate(contractStartDate.format('MM-DD-YYYY'))
		) {
			return;
		}
