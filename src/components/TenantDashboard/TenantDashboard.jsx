            ...
            {selectedProperty?.lease_status === "ACTIVE" ? (
              <Grid container spacing={isMobile ? 1 : 3}>
                <Grid item xs={12} md={4} sx={{ height: !isMobile ? "80vh" : "auto" }}>
                  <AccountBalanceWidget selectedProperty={selectedProperty} selectedLease={selectedLease} propertyAddr={propertyAddr} propertyData={propertyData} total={total} rentFees={rentFees} lateFees={lateFees} utilityFees={utilityFees} setPropertyAddr={setPropertyAddr} setPropertyId={setPropertyId} setSelectedProperty={setSelectedProperty} setSelectedLease={setSelectedLease} setTotal={setTotal}/>
                </Grid>
...
const AccountBalanceWidget = ({ selectedProperty, selectedLease, propertyAddr, propertyData, total, rentFees, lateFees, utilityFees, setPropertyAddr, setPropertyId, setTotal, setSelectedProperty, setSelectedLease,  }) => {
  const navigate = useNavigate();
  const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
  const isMedium = useMediaQuery(theme.breakpoints.down("md"));
    ...
    <DashboardTab fullHeight={!isMobile ? true : false}>
      ...
      >
        <Box
          ...
          onClick={() => {
            navigate("/payments", { state: { accountBalanceWidgetData : 
              { selectedProperty, selectedLease, propertyAddr, propertyData, total, rentFees, lateFees, utilityFees  }
            }});
          }}
        >
          Make a Payment
