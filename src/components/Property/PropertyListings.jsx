...
const FilterButtons = ({
  ...
  const areAnyFiltersSet = Object.values(selectedFilters).some(
    (value) => value !== ""
  );
...
>>>>>>> master
            ...
            <Menu
              ...
              onClose={() =>
                setMenuStates((prev) => ({ ...prev, price: null }))
              }
            >
              {/* The values here are just examples */}
              <MenuItem onClick={() => handleSelect("price", "Low-High")}>
                Low-High
              </MenuItem>
              <MenuItem onClick={() => handleSelect("price", "High-Low")}>
                High-Low
              </MenuItem>
              {selectedFilters.price !== "" ? (
                <MenuItem onClick={() => clearFilters("price")}>Clear</MenuItem>
              ) : null}
            </Menu>
          </Stack>
        </Box>
        <Box>
          <Stack direction="column">
            ...
            >
              {/* The values here are just examples */}
              <MenuItem onClick={() => handleSelect("type", "House")}>
                House
              </MenuItem>
              <MenuItem onClick={() => handleSelect("type", "Apartment")}>
                Apartment
              </MenuItem>
              <MenuItem onClick={() => handleSelect("type", "Condo")}>
                Condo
              </MenuItem>
              <MenuItem onClick={() => handleSelect("type", "Multi Family")}>
                Multi Family
              </MenuItem>
              <MenuItem onClick={() => handleSelect("type", "Single Family")}>
                Single Family
              </MenuItem>
              <MenuItem onClick={() => handleSelect("type", "Townhome")}>
                Townhome
              </MenuItem>
              {selectedFilters.type !== "" ? (
                <MenuItem onClick={() => clearFilters("type")}>Clear</MenuItem>
              ) : null}
            </Menu>
          </Stack>
        </Box>
          ...
          <Stack direction="column">
            ...
            >
              ...
              {selectedFilters.beds !== "" ? (
                <MenuItem onClick={() => clearFilters("beds")}>Clear</MenuItem>
              ) : null}
            </Menu>
          </Stack>
        </Box>
          ...
          <Stack direction="column">
            ...
            >
              ...
              {selectedFilters.bath !== "" ? (
                <MenuItem onClick={() => clearFilters("bath")}>Clear</MenuItem>
              ) : null}
            </Menu>
          </Stack>
        </Box>
...
>>>>>>> master
    ...
    const propertyResponse = await fetch(
      `https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/listings/${getProfileId()}`
    );
    ...
    const leaseData = await leaseResponse.json();
    console.log(
      "leaseData.Lease_Details.result",
      leaseData.Lease_Details.result
    );
    const propertyData = await propertyResponse.json();
    ...
    if (JSON.stringify(leaseData) === "{}") {
      ...
      if (
        !leaseData.Lease_Details?.result ||
        !propertyData.Property_Dashboard?.result
      ) {
        console.error("Data is missing from the API response");
        setShowSpinner(false);
        return;
  ...
  function sortProperties(leaseData, propertyData) {
    if (JSON.stringify(leaseData) !== "{}") {
      ...
      var sortedProperties = [...propertyData]; // Create a shallow copy to avoid mutating the original array
      leases.forEach((lease) => {
        const appliedPropertyIndex = sortedProperties.findIndex(
          (property) => property.property_uid === lease.property_id
        );
        // console.log("applied to property at index", appliedPropertyIndex, lease.lease_status)
        if (appliedPropertyIndex > -1) {
          // Make sure the property was found
          const appliedProperty = sortedProperties.splice(
            appliedPropertyIndex,
            1
          )[0]; // Remove the property and store it
        ...
        >
          ...
          <Stack>
            <SearchBar
              ...
              setFilteredItems={setDisplayProperties}
              sx={{ width: "100%" }}
            />
          </Stack>
          <Stack>
            <FilterButtons
              propertyList={sortedProperties}
              filteredItems={displayProperties}
              setFilteredItems={setDisplayProperties}
            />
          </Stack>
          <Stack
          ...
          {displayProperties.length > 0 &&
            displayProperties.map((property, index) => {
              var status = "";
              const appliedData = tenantLeaseDetails.find(
                (lease) => lease.lease_property_id === property.property_uid
              );
              if (appliedData) {
                console.log(
                  appliedData.lease_status,
                  appliedData.property_area,
                  appliedData.lease_start,
                  appliedData.lease_status
                );
                status = appliedData.lease_status;
                console.log(
                  appliedData.property_address,
                  "lease status",
                  status
                );
              }
...
<<<<<<< HEAD
        ...
        {status === "REJECTED" && profileId === property.lt_tenant_id ? (
          <Box
            ...
            onClick={() =>
              console.log(
                "Clicked Approved Button for Property",
                property,
                "with lease",
                lease,
                "and status",
