...
export default function PropertyNavigator({
	...
	const parsedPropertyImages =
		propertyData && propertyData[currentIndex] && propertyData[currentIndex].property_images
	...
	useEffect(() => {
		...
		const parsedPropertyImages =
			propertyList && propertyList[nextIndex] && propertyList[nextIndex].property_images
	...
	const handleNextCard = () => {
		...
		const parsedPropertyImages =
			propertyData && propertyData[nextIndex] && propertyData[nextIndex].property_images
	...
	const handlePreviousCard = () => {
		let previousIndex =
			(currentIndex - 1 + (propertyData ? propertyData.length : 1)) % (propertyData ? propertyData.length : 1);
		setCurrentIndex(previousIndex);
		const previousId =
			propertyData && propertyData[previousIndex] ? propertyData[previousIndex].property_uid : null;
		setCurrentId(previousId);
		setProperty(propertyData && propertyData[previousIndex]);

		const parsedPropertyImages =
			propertyData && propertyData[previousIndex] && propertyData[previousIndex].property_images
	...
	const rentStatusColumns = [
		...
		},
		{
			...
			renderCell: (params) => {
				...
				return (
					<Box sx={{ width: '100%', color: getLateFeesColor(params.row) }}>
						{params.row.lf_pur_amount_due}
					</Box>
				);
			},
		},
				...
				<Grid container sx={{ marginTop: '15px', alignItems: 'center', justifyContent: 'center' }}>
					<Grid item md={1} xs={2} sx={{ display: 'flex', justifyContent: 'center' }}>
						<Button
							...
							disabled={currentIndex === 0 || !propertyData || propertyData.length === 0}
						>
							{currentIndex === 0 ? (
								<ArrowBackIcon
					...
					<Grid item md={1} xs={2} sx={{ display: 'flex', justifyContent: 'center' }}>
						<Button
							...
							disabled={
								currentIndex === propertyData.length - 1 || !propertyData || propertyData.length === 0
							}
						>
										...
										<Grid container spacing={2} sx={{ height: '100%' }}>
											...
											<Grid item xs={6}>
												...
												>
													{property
														? `$${(
																property.property_value / property.property_area
														  ).toFixed(2)}`
														: '-'}
												</Typography>
											</Grid>
								...
								<Dialog open={open} onClose={handleClose}>
									...
									<DialogContent>
										...
										<TextField
											...
											onChange={(e) =>
												setcurrentApplRow({ ...currentApplRow, appliance_desc: e.target.value })
											}
										/>
										<TextField
											margin="dense"
										...
										<LocalizationProvider dateAdapter={AdapterDayjs}>
											<DatePicker
												label="Purchased On"
												value={
													currentApplRow?.appliance_purchased
														? dayjs(currentApplRow.appliance_purchased)
														: null
												}
												onChange={(e) =>
													setcurrentApplRow({
										...
										<LocalizationProvider dateAdapter={AdapterDayjs}>
											<DatePicker
												label="Installed On"
												value={
													currentApplRow?.appliance_installed
														? dayjs(currentApplRow.appliance_installed)
														: null
												}
												onChange={(e) =>
													setcurrentApplRow({
										...
										<LocalizationProvider dateAdapter={AdapterDayjs}>
											<DatePicker
												label="Warranty Till"
												value={
													currentApplRow?.appliance_warranty_till
														? dayjs(currentApplRow.appliance_warranty_till)
														: null
												}
												onChange={(e) =>
													setcurrentApplRow({
										...
										<TextField
											...
											onChange={(e) =>
												setcurrentApplRow({ ...currentApplRow, appliance_url: e.target.value })
											}
										/>
										<TextField
											margin="dense"
