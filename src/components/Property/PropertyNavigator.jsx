...
const getAppColor = (app) =>
  app.lease_status !== "REJECTED"
    ? app.lease_status !== "REFUSED"
      ? "#778DC5"
      : "#874499"
    : "#A52A2A";
...
export default function PropertyNavigator({
  ...
  const [images, setImages] = useState(
    JSON.parse(propertyData[currentIndex].property_images).length > 0
  ...
  const [activeStep, setActiveStep] = useState(() => {
    const index = images.findIndex(
      (image) => image === propertyData[currentIndex].property_favorite_image
    );
  ...
  const [propertyId, setPropertyId] = useState(
    propertyData[currentIndex].property_uid
  );
  ...
  function getPropertyList(data) {
    ...
    return propertyList.map((p) => {
      ...
      p.applicationsCount = [...p.applications].filter(
        (a) => a.lease_status === "NEW"
      ).length;
      return p;
    });
  }

  //STEP 1. Run endpoint get data from database
  const refreshPropertyData = async () => {
    try {
      ...
      console.log("1: Get endpoint", propertyList);
  ...
  useEffect(() => {
    ...
    const getContractsForOwner = async () => {
      try {
        const response = await fetch(
          `https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/contracts/${getProfileId()}`
        );
        if (!response.ok) {
        ...
        var count = 0;
        const contracts = contractsResponse.result.filter(
  ...
  const manager_detail = item.business_uid
    ? `${item.business_name}`
    : "No Manager";
    ...
    const getMaintenanceForProperty = async () => {
      ...
      try {
        const responseProperty = await fetch(
          `https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/maintenanceByProperty/${item.property_uid}`
        );
        const propertyMaintenanceData = await responseProperty.json();
        let propMaintList =
          propertyMaintenanceData.MaintenanceProjects?.result || [];
        propMaintList = propMaintList.filter(
          (m) =>
            m.maintenance_request_status !== "COMPLETED" &&
            m.maintenance_request_status !== "CANCELLED"
        );
  ...
  useEffect(() => {
    ...
    maintenanceDataCollectAndProcess(
  ...
  function handleOnClickNavigateToMaintenance(row) {
    ...
    console.log(
      "maintenanceReqData[status].maintenance_items",
      maintenanceReqData[status].maintenance_items
    );
  ...
  function displayTopMaintenanceItem() {
    ...
    if (
      ...
      maintenanceData[0].maintenance_request_uid
    ) {
      return (
        <DataGrid
  ...
  function numberOfMaintenanceItems(maintenanceItems) {
    if (maintenanceItems && maintenanceItems.length > 0) {
      return maintenanceItems.filter((mi) => !!mi.maintenance_request_uid)
        .length;
    } else {
      return 0;
    }
  ...
  const handleNextCard = () => {
    ...
    //   setActiveStep(0);
    setActiveStep(
      JSON.parse(propertyData[currentIndex + 1].property_images).findIndex(
        (url) => url === propertyData[currentIndex + 1].property_favorite_image
      )
    );
  };
  ...
  const handleAppClick = (index) => {
    navigate("/tenantApplicationNav", {
      state: { index: index, property: item },
    });
  };
      ...
      >
        ...
        >
          ...
          <Button
            ...
            disabled={currentIndex == propertyData.length - 1}
          >
            ...
            >
              <Typography
                sx={{
                  color: theme.typography.propertyPage.color,
                  fontWeight: theme.typography.propertyPage.fontWeight,
                  fontSize: theme.typography.propertyPage.fontSize,
                }}
                paddingBottom="20px"
              >
                {item.property_address} {item.property_unit},{" "}
                {item.property_city} {item.property_state} {item.property_zip}
              </Typography>
              <Typography
                sx={{
                  color: theme.typography.propertyPage.color,
              ...
              >
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6}>
                    ...
                    >
                      Lease Expiring :{" "}
                      {item.lease_end ? item.lease_end : "No Lease"}
                    </Typography>
                  </Grid>
                  <Grid item xs={4}>
                    <Typography
                  ...
                  <Grid item xs={11}>
                    <div style={{ display: "flex", alignItems: "center" }}>
                      ...
                      >
                        <Badge
                          badgeContent={numberOfMaintenanceItems(
                            maintenanceData
                          )}
                          color="error"
                          sx={{
...
function Contract(props) {
  ...
  return (
    <Typography sx={textStyle}>
      {contract.contract_business_id} {contract.business_name}{" "}
