...
export default function AddListing({}) {
  ...
  //2. Put the data in the data object
  const [propertyData, setPropertyData] = useState(propertyList[index]);
  console.log("2: Pass the data", propertyData);
  ...
  const [favImage, setFavImage] = useState(
    propertyData.property_favorite_image
  );
  ...
  const [petsAllowed, setPetsAllowed] = useState(
    propertyData.property_pets_allowed === 1 ? true : false
  );
  const [depositForRent, setDepositForRent] = useState(
    propertyData.property_deposit_for_rent === 1 ? true : false
  );
  ...
  //5. User enters data
  useEffect(() => {
    console.log("4. Added utilities ", newUtilitiesPaidBy);
  }, [newUtilitiesPaidBy]);

  useEffect(() => {
    console.log("4. Displayed utilities", mappedUtilitiesPaidBy);
  ...
  const reverseUtilitiesMap = new Map(
    Array.from(utilitiesMap, ([key, value]) => [value, key])
  );
  const reverseEntitiesMap = new Map(
    Array.from(entitiesMap, ([key, value]) => [value, key])
  );
  ...
  const mapUIDsToUtilities = (propertyUtilities) => {
    ...
    console.log(
      "----- in mapUIDsToUtilities, mappedUtilities - ",
      mappedUtilities
    );
    return mappedUtilities;
  };

  //NOTE: 3. Get the data from utilitiesObject
  const utilitiesObject = JSON.parse(propertyData.property_utilities);
  console.log("3: Get the data from utilitiesObject", utilitiesObject);

  let utilitiesInUIDForm = {};
  let mappedUtilities2 = {};
  //4. Display data (if no data, set default)
  useEffect(() => {
    if (utilitiesObject) {
      //console.log(
      //  "*****************************************AddListing useEffect*******************************************"
      //);
  ...
  //NOTE: 7. Grab the data the user put in and map it to key value pair object and turn it into a string object
  ...
  const handleUtilityChange = (utility, entity) => {
    ...
    setMappedUtilitiesPaidBy((prevState) => ({
      ...prevState,
      [utility]: prevState.hasOwnProperty(utility)
        ? entity
        : prevState[utility],
    }));
    ...
    setNewUtilitiesPaidBy((prevState) => ({
      ...(prevState.hasOwnProperty(utility)
        ? { ...prevState, [utility]: entity }
        : prevState),
    }));
  };
  ...
  const handleAddUtility = (utility) => {
    ...
    console.log("Note 4: ", updatedMappedUtilities);
  ...
  const handleSubmit = async (event) => {
    ...
    //6. Turn into UID form and make it a string Object
    const utilitiesJSONString = JSON.stringify(
      mapUtilitiesAndEntitiesToUIDs(mappedUtilitiesPaidBy)
    );
    console.log("6. string Object to send backend", utilitiesJSONString);
    ...
    const postUtilitiesData = async () => {
      ...
      promises.push(
        fetch(
          "https://l0h6a9zi1e.execute-api.us-west-1.amazonaws.com/dev/utilities",
          {
            method: "POST",
    ...
    const putUtilitiesData = async () => {
      ...
      const addedUtilitiesJSONString = JSON.stringify(
        mapUtilitiesAndEntitiesToUIDs(newUtilitiesPaidBy)
      );
      ...
      addedUtilitiesFormData.append(
        "property_utility",
        addedUtilitiesJSONString
      );
    ...
    const autoUpdate = async () => {
      ...
      //console.log("6: Updated property list", propertyList);
      setPropertyData(propertyList[index]);
    };

    putData();

    //NOTE: 7. Run put/post endpoint: view in network
    ...
    try {
      ...
      console.log("8: PropertyList after autoUpdate ", propertyList);
                ...
                <Grid item xs={12}>
                  ...
                  >
                    ...
                    <CardMedia
                      component="img"
                      image={
                        selectedImageList[activeStep]
                          ? selectedImageList[activeStep].image
                          : defaultHouseImage
                      }
                      // image={coverImage}
                      sx={{
            ...
            >
              ...
              <Grid container columnSpacing={2} rowSpacing={3}>
                ...
                {Object.entries(mappedUtilitiesPaidBy).length > 0
                  ? Object.entries(mappedUtilitiesPaidBy).map(
                      ([utility, selectedValue]) => (
                            ...
                            <FormControlLabel
                              ...
                              control={
                                <Radio
                                  ...
                                  onChange={
                                    () => handleUtilityChange(utility, "owner") //NOTE: 5. User enters data
                                  }
                                />
                              }
                              label="Owner"
                            ...
                            <FormControlLabel
                              ...
                              control={
                                <Radio
                                  ...
                                  onChange={() =>
                                    handleUtilityChange(utility, "tenant")
                                  }
                                />
                              }
                              label="Tenant"
                  ...
                  : Object.entries(defaultUtilities).map(
                      ([utility, selectedValue]) => (
                        <Fragment key={utility}>
                          <Grid item xs={6}>
                            <Typography
                            ...
                            <FormControlLabel
                              ...
                              control={
                                <Radio
                                  checked={selectedValue === "owner"}
                                  onChange={() =>
                                    handleUtilityChange(utility, "owner")
                                  }
                                />
                              }
                              label="Owner"
            ...
            <Grid item xs={12}>
              ...
              >
                {keysNotInUtilitiesMap.map((utility, index) => (
                  <MenuItem
                    key={index}
                    onClick={() => handleAddUtility(utility)}
                  >
                    {formatUtilityName(utility)}
                  </MenuItem>
                ))}
            ...
            <Grid container>
              <Grid item xs={12}>
                ...
                <Button
                  ...
                  sx={{
                    width: "100%",
                    backgroundColor: theme.typography.formButton.background,
                  }}
                >
                  <Typography
                    sx={{
